...
void send2clinet()
{
    sprintf(set_begin_time,"%d:%d",wat_begin_hour,wat_begin_min);//todo:回传+读取
...
bool time_plus_check(int wat_begin_hour,int wat_begin_min,tm timeinfo){ //这个gap不能超过30!不能让客户操作这个数值
    int time_gap =7; //在五分钟内必须进行反应
    if (wat_begin_min <time_gap){//这些时间状态应该是与,不应该是或
...
float getTemp(String temp) {
  ...
  return (info[3].toInt() * 256 + info[4].toInt()) / 10;
}
...
void check_soil()//检测土壤湿度
{
  ...
  if (data_soil.length() > 0) { //先输出一下接收到的数据
    // Serial.println();
    // Serial.println(data);
...
//todo:需要再写一个判断土壤湿度是否适宜的程序,也从网上读取参数
bool soil_go(){
    check_soil();
    if (soil2wat==1){//表示目前水还在浇水
        return true;
    }
...
void loop()
{
    ...
    { /*接收数据，client.available()这个函数是指服务器有数据回传*/
        ...
        else if(ch.lastIndexOf('%')!=-1){//读取你所需要的最低湿度
            Serial.println("lowest wet check\n");
            soil_moisture_need=ch.substring(0,ch.length()-1).toFloat();
        }     
        else if(ch.lastIndexOf('x')!=-1){//!读取你所需要的最少时间,小心字母重复!
            Serial.println("least time check\n");
            delaytime=ch.substring(0,ch.length()-1).toInt()*60000;//fixme:这个时间不是很对的,需要测试的时候得到相关时间的单位然后进行修改!
            Serial.print(delaytime);
            carwash_time=delaytime*2;
        }
        else if (ch.lastIndexOf(':')!=-1 && ch.lastIndexOf('v')!=-1){//应当输入5:31v
    ...
    Serial.print("out");
    Serial.print(pump_working_flag);
    if (hand_watering_flag == 1 || (auto_watering_flag == 1 && (time2go() || soil_go()) ) ) // fixme:time2go可能需要再大一点；1.算好delay和time2go;2.做一个每天几次，或者上下午几次的东西
    {
        Serial.print("watt");
        Serial.print(pump_working_flag);
        if (pump_working_flag == 1)
